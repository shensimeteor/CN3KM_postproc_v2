...
#  $second_date    = &hh_advan_date($first_date, $every_hour);
  ...
  system("test -d $NCL_WEB_DIR2 || mkdir -p $NCL_WEB_DIR2");
...
#==============================================================================#
 ...
 #         sleep(7); #wait for file write
          foreach $dom (&uniq(@WRF_DOM_ID))  {
              ...
              chdir($reformat_aux3_dir);
              symlink("$ENSPROCS/add_files.ncl", "add_files.ncl");
              $filename_dom="$RUNDIR/$THIS_CYCLE/$dir_name/auxhist3_d0${dom}$SUFFIX";
              $cmd="$ENSPROCS/reformat_aux3.pl $filename_dom aux3_reformatted.nc >& $mylogdir/aux3_reformat_${d}.log";
              print("$cmd \n");
              system("$cmd");
          }
      }
  ...
  sub do_plots_ncl2_v2 {
        ...
        foreach $d (@MDL1) {
            ...
            if ( $d <= $valid_time ){
                ...
                symlink("$workdir/../$d/reformat_aux3/d$wrfdomid/$fn", "$workdir/ncl_OUTPUTS2/d0${domid}/$aux_fn");
            }
        }
        #link ncl
        chdir("$workdir/ncl_OUTPUTS2/d0${domid}/");
...
#        system("mv *log $mylogdir/");
  ...
  sub do_plots_ncl_v2 {
        ...
        if ( -e $fn ) {
            ...
            #only need once
            if( ! $wind_rs_done{$valid_time."d0${wrfdomid}"}) {
                ...
                print "$ncl\n";
                system "$ncl";
                $wind_rs_done{$valid_time."d0${wrfdomid}"}='DONE';
            }
            chdir "$workdir";
            #unlink "$fn";
        }else {
